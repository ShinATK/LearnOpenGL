...
int main()
{
    myWindow.window_Init(800, 600, "4-6-cubemaps");
    ...
    Shader shader(
        relativePath(FILE_FOLDER_PATH, "shader/4-6-vertex.glsl"), 
        relativePath(FILE_FOLDER_PATH, "shader/4-6-frag.glsl")
    );
    Shader skyboxShader(
        relativePath(FILE_FOLDER_PATH, "shader/4-6-skybox_vertex.glsl"),
        relativePath(FILE_FOLDER_PATH, "shader/4-6-skybox_frag.glsl")
    );
    ...
    // load texture
    unsigned int cubeTexture = loadTexture(relativePath(FILE_FOLDER_PATH, "texture/container.jpg"));
    
    // activate shader
    shader.use();
    shader.setInt("texture1", 0); // set sampler2D to Textrue unit 0
...
unsigned int loadCubemap(vector<std::string> faces)
{
    ...
    {
        unsigned char *data = stbi_load(relativePath(TEXTURE_RELATIVE_PATH, faces[i].c_str()), &width, &height, &nrChannels, 0);
        if (data)
        {
